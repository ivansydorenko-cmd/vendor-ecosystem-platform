...
export const createWorkOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (vendor_selection_method === 'auto_notify' || !vendor_selection_method) {
      try {
        const qualifiedVendors = await workOrderService.findQualifiedVendors(
          workOrderId,
          sku.category_id,
          zipcode,
          tenant_id
        ...
        if (vendorIds.length > 0) {
          ...
          logger.info('Vendors notified for work order:', { 
            workOrderId, 
            vendorCount: vendorIds.length 
          });
        } else {
          logger.info('No qualified vendors found for work order:', { workOrderId, zipcode });
        }
      } catch (notifyError) {
        logger.error('Error notifying vendors:', notifyError);
      }
    }
